...
elements. Autonomous elements will contain a transposase gene within the 
...
While CRAN packages will be installed automatically upon downloading 
packFinder, some dependencies must be downloaded from Bioconductor. 
...
Then, packFinder may be installed from github (using the package 
"devtools", available from CRAN).
...
Biostrings provides methods for the conversion of various formats to DNAString 
objects, including from character vectors and fasta files.
...
original element in the cluster. For this data, as there are few clusters, 
...
conserved. For each cluster, tirClust creates a consensus sequence for 
...
autonomous and confirm which matches are likely pack-TYPE elements. Since 
...
have captured genetic material from the host genome. B
