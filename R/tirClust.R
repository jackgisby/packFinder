...
#'
#' @description
#'     Takes transposable elements clustered by 
#'     VSEARCH, \code{\link{packClust}}, and 
#'     produces consensus sequences for the terminal 
#'     inverted repeats of each. Allows for the 
#'     visualisation of TIR similarities between 
#'     clusters for both forward and reverse strands.
#'
#' @param packMatches
...
#'     object, after conversion to a dataframe. Must 
...
#'
#' @param plot
#'     Argument specifying whether the TIR consensus 
#'     sequences should be plottted as a dendrogram.
#'
#' @param plotSavePath
#'     File path for the dendrogram plot. If 
#'     unspecified, the dendrogram plot is not saved.
#'
#' @param k
#'     The k-mer size to be used for calculating 
#'     a distance matrix between TIR consensus sequences. 
#'     See \code{kdistance::kmer}. Larger word sizes 
#'     will not be suitable for longer TIR 
#'     sequences, due to processing time required. 
#'     Additionally k must be greater than the TIR 
#'     sequence length.
#'
#' @param tirLength
#'     The TIR size to be considered. Consensus sequences 
#'     will be generated based on the first and 
#'     last \code{tirLength} bases of a transposon.
#'
#' @param output
#'     Controls the output of \code{tirClust}. 
#'     If output is specified as "consensus", the 
#'     consensus sequences of each TIR cluster will 
#'     be returned; else, if output is specified as 
#'     "dendrogram", a dendrogram object will be 
...
#'
#' @return
#'     If \code{output} is specified as "consensus" 
#'     (default), returns a list of consensus 
#'     sequences for each cluster specified in 
#'     \code{packMatches} as a 
#'     \code{\link[Biostrings:XStringSet-class]{DNAStringSet}}. 
#'     Else if \code{output} is specified as 
#'     "dendrogram", returns a dendrogram object 
#'     used to create hierarchical clustering diagrams.
#'
#' @seealso
