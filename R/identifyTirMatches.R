#' @title Identify TIR Matches
#' @description Searches a DNAStringSet for potential TIRs based on sequence similarity.
#' @param tirMatches A dataframe of previously identified TIRs, in the format generated by \code{\linkGenomicRanges::makeGRangesFromDataFrame}}.
...
identifyTirMatches <- function(tirMatches = NULL,
  ...
  if (is.null(tirMatches)) {
    tirMatches <- data.frame(seqnames = character(),
                             start = integer(),
                             end = integer(),
                             width = integer(),
                             strand = character())
