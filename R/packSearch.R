...
#'
#' @description
#'     General use pipeline function for the 
#'     Pack-TYPE transposon finding algorithm.
#'
#' @param tirSeq
...
#'     for TIR matches (indels + substitions). See 
#'     \code{\link[Biostrings]{matchPattern}} for
#'     details.
#'
#' @param elementLength
#'     The maximum element length to be 
#'     considered, as a vector of two integers.
#'     E.g. \code{c(300, 3500)}
#'
#' @param tsdLength
...
#'     }
#'
#'     The algorithm finds potential forward and 
#'     reverse TIR sequences using 
#'     \code{\link{identifyTirMatches}} and their 
#'     associated TSD sequence via 
#'     \code{\link{getTsds}}. The main filtering 
#'     stage, \code{\link{identifyPotentialPackElements}}, 
#'     filters matches to obtain a dataframe of potential 
#'     PACK elements. Note that this pipeline does 
#'     not consider the possibility of discovered 
#'     elements being autonomous elements, so it is 
#'     recommended to cluster and/or BLAST elements 
#'     for further analysis.Furthermore, only exact 
#'     TSD matches are considered, so supplying long 
#'     sequences for TSD elements may lead to false-negative 
#'     results.
#'
#' @return
#'     A dataframe, \code{packMatches}, containing 
#'     elements identified by thealgorithm. These
#'     may be autonomous or pack-TYPE elements. 
...
#'         \code{Genome} to which \code{start} and 
#'             \code{end} refer to.
#'         \item width - the width of the 
#'             predicted element.
#'         \item strand - the strand direction of 
#'             the transposable element. This will 
#'             be set to "*" as the \code{packSearch} 
#'             function does not consider
#'             transposons to have a direction - only 
#'             TIR sequences. Passing the 
#'             \code{packMatches} dataframe to 
#'             \code{\link{packClust}} will assign a
...
#'     \itemize{
#'         \item Autonomous elements - autonomous 
#'             elements will be predicted by this 
#'             algorithm as there is no BLAST step. 
#'             It is recommended that, after
#'             clustering elements using 
#'             \code{\link{packClust}}, the user 
#'             analyses each group to determine 
#'             which predicted elements are autonomous 
#'             and which are likely Pack-TYPE elements. 
#'             Alternatively, databases such as Repbase
#'             (\url{https://www.girinst.org/repbase/}) 
#'             supply annotations for autonomous 
#'             transposable elements that can be used to 
#'             filter autonomous matches.
#'         \item TSD Mismatches - if two TIRs do 
#'             not have exact matches for their 
#'             terminal site duplications they will 
#'             be ignored. Supplying longer TSD
#'             sequences will likely lead to a lower 
#'             false-positive rate, however may
#'             also cause a greater rate of 
